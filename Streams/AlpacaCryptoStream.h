...
class AlpacaCryptoStream : public IDataStream, public IAsync { // only difference between this and alpacaStream is the parseing in the loop method (crypto data doesn't have exchange info)
    ...
    AlpacaCryptoStream (const AlpacaStream&) = delete;
    AlpacaCryptoStream& operator= (const AlpacaStream&) = delete;
    AlpacaCryptoStream(JSONFileParser& file, std::string  accountJSONKey, std::string _urlPath, int _port){  // pathUrl = /v2/{source} iex or sip to {source} iex is all you get without paying for subscription 
        urlPath = _urlPath;
        port = _port;
